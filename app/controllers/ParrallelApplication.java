...
public class ParrallelApplication extends Controller {
	
	public static Result parrallelAdd(){
		return ok(parralleladd.render(""));
	}
	
	public static Result parrallelAddDo(){
...
//		File dir=new File("../apache-tomcat-8.0.30/webapps/images");
		...
		long now=new Date().getTime();
			...
			Parrallel advert=new Parrallel();
			if(file==null){
		...
		return ok(parralleladd.render(result));
		
	}
	
	public static Result parrallelList() {
    	...
    	List<Parrallel> list = ParrallelApplication.list(pageSize,pageNow,btn);
    	
    	//总数
		int countAll = ParrallelApplication.count(btn);
		...
		return ok(parrallel.render(list,countAll,pageNow,pageCount,pageSize,"parrallel",btn));
    ...
    public static List<Parrallel> list(int pageSize,int pageNow,List<InputBtn>... btn){
		...
		//sql语句       
	 	String sql = "select id from cc_parrallel where 1=1 "+paramsSql+" order by createtime desc limit "+start+" ,"+pageSize;   
    ...
    public static int count(List<InputBtn>... btn){
		...
		//sql语句       
	 	String sql = "select count(id) as count from cc_parrallel where 1=1 "+paramsSql;  
    ...
    public static Result parrallelDel(int id){
    	Parrallel.findById(id).delete();
    	return parrallelList();
    }
    
    public static Result parrallelEdit(int id){
    	Parrallel advert= Parrallel.findById(id);;
    	return ok(parralleledit.render(advert,""));
    }
    
    
    public static Result parrallelEditDo(){
		DynamicForm in= Form.form().bindFromRequest();
		String result="";
...
//		File dir=new File("../apache-tomcat-8.0.30/webapps/images");
		...
		long now=new Date().getTime();
			...
			Parrallel advert=Parrallel.findById(Integer.parseInt(id));
			advert.setImgurl(file.getAbsoluteFile().getName());
			advert.setName(name);
			advert.setRemark(remark);
			advert.update();
    ...
    public static Result parrallelDetailed(int id){
    	Parrallel advert= Parrallel.findById(id);;
    	return ok(parralleldetail.render(advert,""));
    }
